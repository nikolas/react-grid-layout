...
export function moveElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number, isUserAction: ?boolean, preventCollision: ?boolean): Layout {
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (preventCollision) {
      const stickyArea = 40;
      const checkRight = Math.abs(l.x - (collision.x + collision.w));
      const checkLeft = Math.abs((l.x + l.w) - collision.x);
      if (checkLeft < stickyArea && checkLeft < checkRight && collisions.length === 1) {
        l.x = collision.x - l.w;
      } else if (checkRight < stickyArea && checkRight < checkLeft && collisions.length === 1) {
